...
when they run Python bytecode, which means CPU-bound code gets no parallelism with threads. 
Processes avoid the GIL because each process has its own interpreter, so CPU-intensive work 
scales with the number of cores. [to avoid loop of multiple process importing modules and running 
functions , use the guard __name__=‘__main__’ so your function only runs once when you run `python 
script.py `]
For I/O-bound operations like HTTP requests, the CPython’s GIL is released during network waits, 
and threads are generally faster because they are lightweight and avoid the overhead of process 
creation, pickling[convert object into byte stream to send to another process], and inter-process 
communication. In your code, the work is purely network I/O, so ProcessPoolExecutor provides 
no improvement and usually performs worse.

brief: if `I/O bound` then use thread; elif cpu-bound GIL block thread so use process with dunder guard
'''
...
# In a concise narrative, yield creates a generator, while async/await creates a coroutine managed by 
# an event loop. A generator pauses to produce a sequence of values one at a time, and the caller 
# drives it by repeatedly asking for the next value. A coroutine pauses to wait for an asynchronous 
# operation, and the event loop—not the caller—resumes it when the awaited task completes
